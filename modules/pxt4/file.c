...
static bool pxt4_dio_supported(struct inode *inode)
{
...
static ssize_t pxt4_dio_read_iter(struct kiocb *iocb, struct iov_iter *to)
{
	...
	ret = iomap_dio_rw(iocb, to, &pxt4_iomap_ops, NULL,
			   is_sync_kiocb(iocb));
	inode_unlock_shared(inode);
					   ...
					   ssize_t written, size_t count)
{
	 ...
	 */
	WARN_ON_ONCE(i_size_read(inode) < PXT4_I(inode)->i_disksize);
				 ...
				 int error, unsigned int flags)
{
	...
	if (size && flags & IOMAP_DIO_UNWRITTEN)
		return pxt4_convert_unwritten_extents(NULL, inode,
						      offset, size);
...
static ssize_t pxt4_dio_write_iter(struct kiocb *iocb, struct iov_iter *from)
{
	...
	if (pxt4_test_inode_flag(inode, PXT4_INODE_EXTENTS) &&
	    !is_sync_kiocb(iocb) && pxt4_unaligned_aio(inode, from, offset)) {
		unaligned_aio = true;
	 ...
	 * dioread_nolock path.
	 */
	if (!unaligned_aio && pxt4_overwrite_io(inode, offset, count) &&
	    pxt4_should_dioread_nolock(inode)) {
		overwrite = true;
	...
	ret = iomap_dio_rw(iocb, from, &pxt4_iomap_ops, &pxt4_dio_write_ops,
			   is_sync_kiocb(iocb) || unaligned_aio || extend);

	if (extend)
		ret = pxt4_handle_inode_extension(inode, offset, ret, count);
